...
public class Unit extends GoodsLocation
	 ...
	 * @param leaveColony  True if the unit is about to leave the colony, not just the education building.
	 * @param gUI
	 * @return  True if the unit can proceed.
	 */
	public boolean confirmAbandonEducation(boolean leaveColony, GUI gUI) {
		if (!isInColony())
			return true;
		boolean teacher = getStudent() != null;
		boolean student = leaveColony && getTeacher() != null;
		if (!teacher && !student)
			return true;
		Building school = (Building) ((teacher) ? getLocation() : getTeacher().getLocation());
		StringTemplate label = getLabel(Unit.UnitLabelType.NATIONAL);
		...
		return template == null
				|| gUI.confirm(getTile(), template, this, "abandonEducation.yes", "abandonEducation.no");
	}
}
