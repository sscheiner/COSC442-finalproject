...
public class IndianSettlement extends Settlement implements TradeLocation {
	...
	/**
	 * Gets a chip image for the alarm at an Indian settlement. The background is either the native owner's, or that of the most hated nation if any.
	 ...
	 */
	public BufferedImage getAlarmChip(Graphics2D g, Player player, ImageLibrary imageLibrary) {
		if (player == null || !hasContacted(player))
			return null;
		Color ownerColor = getOwner().getNationColor();
		Player enemy = getMostHated();
		Color enemyColor = (enemy == null) ? Nation.UNKNOWN_NATION_COLOR : enemy.getNationColor();
		int amount = 4;
		if (enemy == null) {
			amount = 0;
		} else if (player == enemy) {
			Tension alarm = getAlarm(enemy);
		...
		String text = ResourceManager
				.getString((worthScouting(player)) ? "indianAlarmChip.contacted" : "indianAlarmChip.scouted");
		return imageLibrary.createFilledChip(g, text, Color.BLACK, ownerColor, amount / 4.0, enemyColor, foreground);
	}
	 ...
	 * @param gUI
	 * @return  The amount of tribute to demand, positive if the demand should proceed.
	 */
	public int confirmNativeTribute(Unit attacker, GUI gUI) {
		...
		String messageId = (other.getNumberOfSettlements() >= strength) ? "confirmTribute.unwise"
				: (other.getStance(player) == Stance.CEASE_FIRE) ? "confirmTribute.warLikely"
						: (getAlarm(player).getLevel() == Tension.Level.HAPPY) ? "confirmTribute.happy"
								: "confirmTribute.normal";
		return (gUI
				.confirm(getTile(),
						StringTemplate.template(messageId).addName("%settlement%", getName())
